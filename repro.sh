...
# deptry complains about pyproject.toml not being found when ROOT isn't ".", bug?
# "$SCRIPT_DIR/venv/bin/python" -m deptry "$SCRIPT_DIR/company.bar" 

echo "Running deptry on company.foo..."
$(cd "$SCRIPT_DIR/company.foo" && "$SCRIPT_DIR/venv/bin/python" -m deptry .)
